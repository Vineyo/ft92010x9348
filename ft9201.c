...
MODULE_DESCRIPTION("FT9201 Fingerprint reader driver");

#define VENDOR_ID 0x2808
#define PRODUCT_ID 0x9348
...
static long ft9201_ioctl(struct file* file, unsigned int cmd,
    unsigned long arg);
static ssize_t ft9201_read(struct file* fp, char __user* buf, size_t count,
    loff_t* f_pos);
...
static int ft9201_initialize(struct ft9201_device* dev)
{
    ...
    dev_info(&dev->interface->dev, "Image dimensions: %d x %d",
        dev->device_status.sensor_width, dev->device_status.sensor_height);
...
static int ft9201_read_image(struct ft9201_device* dev)
{
    ...
    dev_info(&dev->interface->dev,
        "Reading image from scanner; dimensions: %dx%d",
        dev->device_status.sensor_width, dev->device_status.sensor_height);
    ...
    retVal = usb_bulk_msg(
        ...
        img_with_header, transfer_size, &read_length, USB_READ_OP_TIMEOUT);
    if (retVal < 0) {
        dev_err(&dev->interface->dev, "Error reading data from device: Error %d",
            retVal);
        goto out;
    }
...
{
    dev_info(&dev->interface->dev, "Copied: %lu, Filled: %lu", dev->img_in_copied,
        dev->img_in_filled);
...
static ssize_t send_read_data(struct ft9201_device* dev, char __user* buf,
    size_t count)
{
    ...
    dev_info(&dev->interface->dev, "Copied: %lu, to_copy: %lu, full_size: %lu",
        dev->img_in_copied, to_copy, dev->img_in_size);

    if (dev->img_in_copied + to_copy > dev->img_in_size) {
        dev_info(&dev->interface->dev,
            "error img_in_copied + to_copy > dev->img_in_size");
        return -EINVAL;
    }
    ...
    loff_t* f_pos)
{
    ...
    while (true) {
        ...
        */
        ...
        retval = usb_control_msg_send(dev->udev, 0, 52, 0x40, 0x0003, 0, NULL, 0,
            5000, GFP_KERNEL);
        ...
        retval = usb_control_msg_send(dev->udev, 0, 111, 0x40, 0x0020, 37248, NULL,
            0, 5000, GFP_KERNEL);
        ...
        retval = usb_control_msg_send(dev->udev, 0, 111, 0x40, 0x1400, 36992, NULL,
            0, 5000, GFP_KERNEL);
        ...
        while (poo == 0) {
            retval = usb_control_msg_recv(
                dev->udev, 0, FT9201_REQ_READ_REGISTERS,
                USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0, 0, &local_value,
                sizeof(local_value), 5000, GFP_KERNEL);
            ...
            //	dev_info(&dev->interface->dev, "read registers returned: %d %d
            //%d %d\n", local_value[0], local_value[1], local_value[2],
            // local_value[3]);
...
static int ft9201_probe(struct usb_interface* intf,
    const struct usb_device_id* id)
{
    ...
    // bulk_out endpoint is not yet used. It might be used only for upgrading
    // firmware if it's ever implemented
    /* use only the first bulk-in and bulk-out endpoints */
    retval = usb_find_common_endpoints(intf->cur_altsetting, &bulk_in, &bulk_out,
        NULL, NULL);
    if (retval) {
    ...
    dev_info(&intf->dev, "USB fpreader device now attached to fpreader%d",
        intf->minor);
